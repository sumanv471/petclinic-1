						 ...
						 sshPublisher(
                                            ...
                                            publishers:  [
                                                sshPublisherDesc(
                                                    ...
                                                    transfers:[   
                                                        sshTransfer(
								execCommand:'docker stop "$(docker ps -a -q)" && docker rm -f "$(docker ps -a -q)" && cd docker && docker system prune -a --force && docker build -t petclinic . --no-cache && docker run -itd --name petclinic -p 8080:8080 petclinic'
                                                        )
                                                    ]
                                                )
                                            ]
